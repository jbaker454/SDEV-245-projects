class Cipher():
  ...
  def rotate(self, x:int, n:int): 
    return ((x >> n) | (x << (32 - n))) & 0xffffffff
  def shift(self, x:int, n:int): 
    return x >> n

  def choice(self, x:int, y:int, z:int): 
    #takes 3 0s and 1s and chooses the minority
    return (x & y) ^ (~x & z)
  def majority(self, x:int, y:int, z:int): 
    #takes 3 0s and 1s and chooses the majority
    return (x & y) ^ (x & z) ^ (y & z)

  def bigSigma0(self, x:int): 
    return self.rotate(x, 2) ^ self.rotate(x, 13) ^ self.rotate(x, 22)
  def bigSigma1(self, x:int): 
    return self.rotate(x, 6) ^ self.rotate(x, 11) ^ self.rotate(x, 25)
  def littleSigma0(self, x:int): 
    return self.rotate(x, 7) ^ self.rotate(x, 18) ^ self.shift(x, 3)
  def littleSigma1(self, x:int): 
    return self.rotate(x, 17) ^ self.rotate(x, 19) ^ self.shift(x, 10)

  def determineMessage(self):
    message = input("input message here: ")
    return message

  def padByteMessage(self, byteMessage: bytes):
  ...
  def processBlock(self, block: bytes, currentState: list[int]):
    W = [int.from_bytes(block[j:j+4], 'big') for j in range(0, 64, 4)]
    for t in range(16, 64):
      val = (self.littleSigma1(W[t-2]) + W[t-7] + self.littleSigma0(W[t-15]) + W[t-16]) & 0xffffffff
      W.append(val)

    a,b,c,d,e,f,g,h = currentState
    for t in range(64):
      T1 = (h + self.bigSigma1(e) + self.choice(e,f,g) + self.K[t] + W[t]) & 0xffffffff
      T2 = (self.bigSigma0(a) + self.majority(a,b,c)) & 0xffffffff
      h = g
      g = f
      f = e
      e = (d + T1) & 0xffffffff
  ...
  def processMessage(self, message:str) -> str:
    ...
    byteMessage = self.padByteMessage(byteMessage)
    
    currentState = self.startState
    for i in range(0, len(byteMessage), 64):
      block = byteMessage[i:i+64]
      currentState = self.processBlock(block, currentState)
